...
def log_conversation_trace_hierarchical(count, hierarchical_row, agent_names, csv_path="conversation_trace_qwen_mathqa.csv"):
    """
    Logs the output of a hierarchical pipeline.

    Parameters:
    - count: Index of the current example
    - hierarchical_row: List of agent outputs in the same order as agent_names
    - agent_names: List of subagent names (e.g. ["problem_understanding", "mathematical_formulation", ...])
    - csv_path: Output CSV path
    """
    assert len(agent_names) == len(hierarchical_row), "Mismatch between agents and outputs"
    ...
    for agent, output in zip(agent_names, hierarchical_row):
        row[agent] = output
